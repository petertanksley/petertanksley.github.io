...
# impact-factor pips (2026-09-19): an article reads its venue's Clarivate JIF for the JCR year JIF_LAG before its own
# year (the number that was public when it appeared). Pip k lights when JIF >= IMPACT_TIERS[k]. If that JCR year has
# no JIF (emerging-sources journals before JCR 2022) the nearest available year is used and flagged. Data:
# 5_skilltree/data/journals.yml, generated by build_journals.R from the JCR exports in data/jcr/.
JIF_LAG      <- 1
...
journals_file <- here("5_skilltree", "data", "journals.yml")
journals <- if (file.exists(journals_file)) read_yaml(journals_file) else { warning("no journals.yml (run build_journals.R); no impact pips"); list() }
journal_of <- setNames(journals, map_chr(journals, "venue"))
ids <- map_chr(arts, "id")
problems <- character()
...
for (a in arts) {
  ...
  if (a$status != "preprint" && length(journals) && (is.null(journal_of[[a$venue]]) || !length(journal_of[[a$venue]]$jif)))
    note("%s: venue '%s' has no JIF data in journals.yml (no impact pips)", a$id, a$venue)
}
hard <- problems[!grepl("not rated|no JIF data", problems)]
...
impact_of <- function(a) {
  j <- journal_of[[a$venue]]
  if (a$status == "preprint" || is.null(j) || !length(j$jif)) return(NULL)
  yrs <- as.integer(names(j$jif)); vals <- map_dbl(j$jif, ~ if (is.null(.x)) NA_real_ else as.numeric(.x)); ok <- !is.na(vals)   # keep NULL years aligned
  if (!any(ok)) return(NULL)
  want <- a$year - JIF_LAG
  use  <- if (want %in% yrs[ok]) want else { cand <- yrs[ok]; cand[order(abs(cand - want), cand)][1] }   # nearest; ties go to the earlier year
  jif  <- vals[yrs == use]
  list(jif = jif, jcr_year = use, requested_year = want, nearest = use != want,
       tier = sum(jif >= IMPACT_TIERS), quartile = na_if_null(j$quartile[[as.character(use)]]))
}
...
imp <- map(nodes, "impact"); tiers <- map_int(imp, ~ if (is.null(.x)) 0L else as.integer(.x$tier))
